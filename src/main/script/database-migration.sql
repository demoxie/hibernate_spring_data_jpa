...
CREATE DATABASE IF NOT EXISTS `bookadmin `@` %` IDENTIFIED WITH mysql_native_password BY 'password';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, REFERENCES , INDEX,ALTER, EXECUTE, CREATE VIEW, SHOW VIEW,
CREATE ROUTINE, ALTER ROUTINE, EVENT, TRIGGER ON `bookdb`.* TO `bookadmin `@` %`;
CREATE DATABASE IF NOT EXISTS `bookuser `@` %` IDENTIFIED WITH mysql_native_password BY 'password';
